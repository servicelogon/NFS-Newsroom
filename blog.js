...
export function tagPage(label, posts, seo = {}) {
  ...
  return layout(label, `Field notes tagged ${label}.`, 'Blog', `
    ...
    <aside class="explore-strip"><p>Keep exploring.</p><a class="internal-button" href="/">All field notes</a></aside>`, { path: `/blog/tag/${tagSlug(label)}`, origin: seo.origin || '', image: posts.find(post => post.image)?.image || null });
}
export function tagNotFoundPage(seo = {}) {
  return layout('Tag not found', 'No published field notes use this tag.', 'Blog', '<section class="empty-state"><p class="kicker">404 / OFF THE MAP</p><h1>No notes under that tag.</h1><p>That tag is not on any published field note.</p><a class="internal-button" href="/">Back to the blog</a></section>', { path: seo.path || '/blog/tag', origin: seo.origin || '', robots: 'noindex' });
}
export function notFoundPage(seo = {}) {
  return layout('Post not found', 'This field note could not be found.', 'Blog', '<section class="empty-state"><p class="kicker">404 / OFF THE MAP</p><h1>This trail ends here.</h1><p>That post may have moved or is still being written.</p><a class="internal-button" href="/">Back to the blog</a></section>', { path: seo.path || '/blog', origin: seo.origin || '', robots: 'noindex' });
}
export function toolsPage(seo = {}) {
  return layout('Tools', 'Explore Copilot Security Trail and Passkey AAGUID Lookup, tools by Nathan Hess.', 'Tools', `
    ...
    </section><aside class="explore-strip"><p>Built to be useful. Shared to be explored.</p><a class="internal-button" href="/">Back to the field notes</a></aside>`, { path: '/tools', origin: seo.origin || '' });
}
